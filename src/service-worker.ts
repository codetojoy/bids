...
const CACHE = `bids-${version}`;
// `base` is '' at the root and e.g. '/bids' for subpath hosting, so the app-shell
// entry and offline fallback below resolve correctly either way. `.nojekyll` ships in
// static/ only to disable Jekyll on GitHub Pages — it is not an app asset and a host
// may not serve a request for it, so exclude it from the precache (cache.addAll rejects
// on any failed fetch, which fails the install).
const ASSETS = [
	...build,
